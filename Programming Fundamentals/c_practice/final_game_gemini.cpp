...
// Map string rank (user input) to face index
map<string, int> rankStringtoFaceIndex = {
    {"A", 0}, {"2", 1}, {"3", 2}, {"4", 3}, {"5", 4},
    {"6", 5}, {"7", 6}, {"8", 7}, {"9", 8}, {"10", 9},
    {"J", 10}, {"Q", 11}, {"K", 12}
};

// Map string suit (user input) to suit index
map<string, int> suitStringtoSuitIndex = {
...
    // Adjust spacing for "10"
    if (rank_str.length() == 1) {
        card[1] = "| " + rank_str + SPACE_11 + "|";
        card[6] = "| " + SPACE_11 + rank_str + " |";
    } else { // For "10"
        card[1] = "| " + rank_str + SPACE_10 + "|";
        card[6] = "| " + SPACE_10 + rank_str + " |";
    }
...
void discard_cards(vector<vector<int>>& current_hand, const vector<vector<int>>& cards_to_discard) {
    // We create a new hand containing only the cards we want to keep
    vector<vector<int>> new_hand;
    for (const auto& existing_card : current_hand) {
        bool should_discard = false;
        for (const auto& discard_card : cards_to_discard) {
            if (existing_card[0] == discard_card[0] && existing_card[1] == discard_card[1]) {
                should_discard = true;
                break;
            }
        }
        if (!should_discard) {
            new_hand.push_back(existing_card);
        }
    }
    current_hand = new_hand; // Replace the old hand with the new one
}
...
    int num_to_draw = 0; // Number of cards computer will discard and redraw
    vector<vector<int>> cards_to_discard; // To collect cards for discard
...
        // Sort hand to easily pick lowest cards
        vector<vector<int>> sorted_hand = current_hand;
        sort(sorted_hand.begin(), sorted_hand.end(), [](const vector<int>& a, const vector<int>& b) {
            return a[1] < b[1]; // Sort by face rank
        });

        for (int i = 0; i < num_to_draw && i < sorted_hand.size(); ++i) {
            cards_to_discard.push_back(sorted_hand[i]);
        }
    }

    // Perform discard
    if (!cards_to_discard.empty()) {
...
    cout << "Enter cards to discard (e.g., '2 of Hearts', 'J of Diamonds'). Type 'done' when finished." << endl;
...
        // Parse card input (e.g., "A of Hearts", "10 of Spades")
        stringstream ss(input_line);
        string rank_str_input, of_str, suit_name_str;
        ss >> rank_str_input >> of_str >> suit_name_str;

        // Convert rank string (e.g., "A", "2", "J", "10") to face_idx
        int discard_face_idx = -1;
        if (rankStringtoFaceIndex.count(rank_str_input)) {
            discard_face_idx = rankStringtoFaceIndex[rank_str_input];
        }

        int discard_suit_idx = -1;
        // Convert string suit name to suit_idx
        if (suitStringtoSuitIndex.count(suit_name_str)) {
            discard_suit_idx = suitStringtoSuitIndex[suit_name_str];
        }
...
                        found_in_hand = true;
                        cout << "Marked for discarding: " << getRankString(discard_face_idx) << " of " << getSuitString(discard_suit_idx) << endl;
                        break;
                    }
                }
            }
...
            }
        } else {
            cout << "Could not parse card. Please use 'Rank of Suit' (e.g., 'A of Hearts', '10 of Spades')." << endl;
        }
    }
...
    // plus at least one card for potential draws (we need at least 1 card in deck to allow a discard and redraw of 1 card).
    while (total_cards_in_deck >= POKER_HAND_SIZE * 2 + 1) { // Minimum cards for 2 hands (5 each) + 1 for potential draw
        clear_screen();
        cout << "--- NEW ROUND ---" << endl;
...
        cout << "Cards Left in Deck: " << total_cards_in_deck << endl;
        // The game loop condition handles continuation, so we remove the prompt to quit here.
    }
