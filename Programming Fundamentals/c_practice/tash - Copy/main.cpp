...
struct Button
{
    ...
    Button(float x, float y, float width, float height, const std::string &name, const std::string &displayTxt, const sf::Font &font)
        // FIX: Initialize sf::Text using its constructor that takes sf::String, sf::Font, and character size.
        // std::string needs to be converted to sf::String.
        : text(sf::String(displayTxt), font, 16) // Initialize text in the member initializer list
    {
        // FIX: sf::RectangleShape::setPosition expects an sf::Vector2f.
        ...
        // These lines are now redundant because 'text' is initialized in the member initializer list above.
        // text.setFont(font);
        ...
        text.setPosition(sf::Vector2f(x + width / 2.0f, y + height / 2.0f)); // Also use sf::Vector2f here
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        // Reverted to original drawText calls with explicit positions
        drawText(window, "Computer's Hand", {300, 35}, 24);
        drawHand(window, computer_cards, 60, currentGameState != GAME_OVER && currentGameState != EVALUATING);

        drawText(window, "Player's Hand", {300, 425}, 24);
        ...
        // Reverted to original drawText call for status messages
        sf::Color statusColor = (currentGameState == GAME_OVER) ? sf::Color::Red : sf::Color::White;
        drawText(window, statusMessage, {300, 385}, 24, statusColor);
        ...
        if (currentGameState == GAME_OVER)
        {
            ...
            // Reverted to original drawText call for final message
            drawText(window, finalMessage, {300, 345}, 40, sf::Color::Yellow);
        }
